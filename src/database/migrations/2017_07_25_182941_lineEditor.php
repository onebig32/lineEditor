...
class LineEditor extends BaseMigration
{
    ...
    public function up()
    {
        ...
        if (!$this->schema->hasTable($tableName)) {
            ...
            DB::connection('mysql_migrate')->statement("ALTER TABLE `kkt_line` COMMENT = '线路行程表';");
        }
        ...
        if (!$this->schema->hasTable($tableName)) {
            $this->schema->create($tableName, function ($table) {
                ...
                $table->integer('tour_id')->comment('线路方案id,关联kkt_line的id');
                $table->string('title')->default('')->comment('线路名称');
            ...
            DB::connection('mysql_migrate')->statement("ALTER TABLE `kkt_line_day` COMMENT = '线路行程天数表';");
        }

        $tableName = "kkt_line_tour";
        if (!$this->schema->hasTable($tableName)) {
            $this->schema->create($tableName, function ($table) {
                $table->increments('id')->unsigned()->comment('表主键id');
                $table->unsignedTinyInteger('is_delete')->default(1)->comment('是否删除 1：否 2：删除');
            ...
            DB::connection('mysql_migrate')->statement("ALTER TABLE `kkt_line_tour` COMMENT = '线路行程方案表';");
        }

        $tableName = "kkt_line_tour_item";
        if (!$this->schema->hasTable($tableName)) {
            $this->schema->create($tableName, function ($table) {
                $table->increments('id')->unsigned()->comment('表主键id');
                $table->integer('tour_id')->comment('线路方案id,关联kkt_line_tour的id');
                $table->integer('item_id')->comment('单项资源项目id,关联kkt_line_item的id');
                $table->unsignedTinyInteger('is_delete')->default(1)->comment('是否删除 1：否 2：删除');
                $table->timestamps();
            });
            DB::connection('mysql_migrate')->statement("ALTER TABLE `kkt_line_tour_item` COMMENT = '线路行程方案单项资源关系表';");
        }
        ...
        if (!$this->schema->hasTable($tableName)) {
            ...
            DB::connection('mysql_migrate')->statement("INSERT INTO kkt_line_item_type (`id`, `pid`, `name`)
            VALUES
            (1, 0, '景点'),
	(2, 0, '娱乐'),
	(3, 0, '购物'),
...
");
        ...
        $tableName = "kkt_line_item_img";
        if (!$this->schema->hasTable($tableName)) {
            $this->schema->create($tableName, function ($table) {
                $table->increments('id')->unsigned()->comment('表主键id');
                $table->integer('item_id')->comment('行程单项id,关联kkt_line_item的id');
                $table->unsignedTinyInteger('cover')->default(1)->comment('是否作为产品封面图片 0否 1:是');
                $table->string('large_url')->default('')->comment('类型名称');
                $table->string('middle_url')->default('')->comment('类型名称');
                $table->string('small_url')->default('')->comment('类型名称');
                $table->string('group_large')->default('')->comment('类型名称');
                $table->string('group_middle')->default('')->comment('类型名称');
                $table->string('group_small')->default('')->comment('类型名称');
                $table->unsignedTinyInteger('is_delete')->default(1)->comment('是否删除 1：否 2：删除');
                $table->timestamps();
                $table->integer('version_id')->comment('版本号id');
            });
            DB::connection('mysql_migrate')->statement("ALTER TABLE `kkt_line_item_img` COMMENT = '线路行程单项资源图片表';");
        }
    ...
    public function down()
    {
        ...
        if ($this->schema->hasTable('kkt_line_tour')) {
            $this->schema->drop('kkt_line_tour');
        }
        if ($this->schema->hasTable('kkt_line_tour_item')) {
            $this->schema->drop('kkt_line_tour_item');
        }
        ...
        if ($this->schema->hasTable('kkt_line_item_img')) {
            $this->schema->drop('kkt_line_item_img');
        }
